...
HOTEL_CATEGORY_KEYWORDS = {
    '蔬菜类': [
        ...
        '洋葱', '蒜苗', '上海青', '包菜', '小白菜', '蘑菇', '金针菇', '平菇', '杏鲍菇',
        ...
        '园茄子', '圆茄子', '白豆角', '扁豆角', '龙豆', '长江豆', '长豇豆', '豇豆', '紫甘蓝', '豆苗', '玉米', '磨菇', '海鲜菇', '藕片', '苦瓜', '西芹',
    ],
...
HOTEL_CATEGORY_OVERRIDES = {
    '调料咸菜类': ['六六红火锅料', '火锅料', '精品魔芋', '素鸡', '泡椒', '东古一品鲜酱油', '蒸鱼豉油', '辣鲜露', '鸡汁', '小米辣', '老干妈', '甜面酱', '番茄酱', '蕃茄酱', '卤肉料包', '麻婆豆腐调料', '麻婆豆腐料', '火锅底料', '酱黄瓜', '裙带菜咸菜', '咸菜', '十三香', '杏仁片', '白芝麻', '黄豆', '虾皮', '红99', '桂皮', '花椒', '白胡椒粉', '白胡椒', '干辣椒', '辣椒段', '辣皮子', '红醋', '生抽', '老抽', '料酒', '蚝油', '耗油', '食盐', '冰糖'],
    ...
    '其他类': ['六六红', '水晶粉', '火锅宽粉', '火锅邵皮', '火锅苕皮', '芝麻酱', '丘比焙煎芝麻酱', '贡菜', '针金菇', '金针菇', '青岛纯生', '鞭炮笋', '香铃卷', '鲜玉米', '鲜虾', '榨菜丁', '榨菜丝', '蒜米', '泡椒', '冰糖', '薄荷叶', '椰果', '蛋黄派', '打包袋', '豆瓣酱'],
}
...
PURCHASE_SUPPLIER_GROUPS = {
    ...
    '王伟供应商': [
        ...
        '平菇', '黄瓜', '西兰花', '西蓝花', '红萝卜', '胡萝卜', '洋葱', '净蒜', '生姜', '姜',
    ],
    '自采': [
        '纯牛奶', '牛奶', '水果玉米', '青笋', '新蒜', '蒜苗', '白萝卜', '象牙萝卜',
    ],
}
...
PURCHASE_SUPPLIER_ITEM_ORDER = {
    ...
    '平菇': 106, '黄瓜': 107, '西兰花': 108, '西蓝花': 108, '红萝卜': 109, '胡萝卜': 109,
    '洋葱': 110, '净蒜': 111, '生姜': 112, '姜': 112,
    '纯牛奶': 201, '牛奶': 201, '水果玉米': 202, '青笋': 203,
    '新蒜': 209, '蒜苗': 211, '白萝卜': 212, '象牙萝卜': 212,
}
...
def supplier_order_group(name):
    ...
    if category == '主食面点类':
        return '面供应商'
    if any(keyword in text for keyword in ['鸡蛋', '鸭蛋', '咸鸭蛋']):
        return '鸡蛋供应商'
    if category == '肉蛋类':
        return '肉供应商'
    if category == '豆制品类':
